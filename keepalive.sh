...
#构建消息配置文件
makeMsgConfig() {
	if [ -n "$TELEGRAM_TOKEN" ] || [ -n "$WXSENDKEY" ] || [ -n "$WXPUSH_URL" ]; then
		if [[ "$TELEGRAM_TOKEN" != "null" || "$WXSENDKEY" != "null" || "$WXPUSH_URL" != "null" ]]; then
			cat >msg.json <<EOF
   {
      "telegram_token": "$TELEGRAM_TOKEN",
      "telegram_userid": "$TELEGRAM_USERID",
      "wxsendkey": "$WXSENDKEY",
      "wxpush_url": "$WXPUSH_URL",
      "wx_token": "$WX_TOKEN",
      "sendtype": "$sendtype",
...
EOF
		...
		echo "TELEGRAM_TOKEN, WXSENDKEY 和 WXPUSH_URL 变量均未设置"
	fi
}
